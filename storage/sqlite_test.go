...
func TestListAndCounts(t *testing.T) {
	...
	all, err := s.ListJobs(ctx, false, false, time.Time{}, time.Time{}, true)
	if err != nil {
...
func TestSearchJobs(t *testing.T) {
	...
	results, _ := s.SearchJobs(ctx, "alpha", nil, time.Time{}, time.Time{}, true)
	if len(results) != 1 || results[0].JobId != "alpha-1" {
		t.Fatalf("results: %+v", results)
	}
	results, _ = s.SearchJobs(ctx, "rm -rf", nil, time.Time{}, time.Time{}, true)
	...
	results, _ = s.SearchJobs(ctx, "sample-A.fastq", nil, time.Time{}, time.Time{}, true)
	if len(results) != 1 || results[0].JobId != "gamma-1" {
		t.Fatalf("input-file search: %+v", results)
	}
	results, _ = s.SearchJobs(ctx, "clean/sample", nil, time.Time{}, time.Time{}, true)
	if len(results) != 1 || results[0].JobId != "gamma-1" {
	...
	results, _ = s.SearchJobs(ctx, "2025-q2", nil, time.Time{}, time.Time{}, true)
	if len(results) != 2 {
