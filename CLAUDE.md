...
- `batchq submit [flags] [script-path | -- inline cmd…]` (or pipe a script on stdin) — enqueue a job. Core flags: `--name`, `-p/--procs`, `-m/--mem`, `-t/--walltime`, `--wd`, `--stdout`, `--stderr`, `--deps afterok:<id>`, `--hold`, `--env`, `--slurm` (parse `#SBATCH` headers), `--array <spec>`, `--resource <name[=val]>`, `--custom <key=val>` (scheduler passthrough), `--run-id`, `--input`, `--output`. Prints the job id (or array id for `--array`).
...
- `batchq web [--socket <path> | --listen host:port] [--force] [--api]` — HTML UI over the same REST client. `--api` turns it into a combined **gateway**: the browser UI at `/` (HTTP Basic, user `admin` + `[web] password`/`BATCHQ_PASSWORD`) and the REST API reverse-proxied under `/api/` (bearer `[batchq] token`/`BATCHQ_TOKEN`), both on one listener. The proxy forwards to the same backend the web process is a client of (autospawned local server, or `[batchq] remote`), so the single-DB-owner invariant holds — the web process never touches the DB. See `web/auth.go` (`requireBasic`/`requireBearer`) and `client.NewReverseProxy`.
...
- Generic required resources: `--resource <name[=value]>` (repeatable), mirrored by `#BATCHQ -resource`. Stored as `job_details` keys under the `resource.` prefix (`jobs.ResourcePrefix`). Count-vs-label is inferred from the value: an integer value (`gpu=2`, `gpu:a100=2`) is a countable requirement matched with `>=`; a non-integer value (`cluster=xyz`) is a label matched by set membership; a bare `name` is a feature flag. `procs`/`mem`/`walltime` are reserved names (rejected — use `-p/-m/-t`). Runners advertise what they have via `[simple_runner]`/`[slurm_runner] resources` config or `batchq run --resource`; the storage-side `jobFitsResources` (in the atomic claim) does the matching. A runner advertising nothing only claims jobs that require nothing. **SLURM emission:** the SLURM runner also translates a claimed job's `resource.*` into sbatch allocation directives (`runner/slurm.go:slurmResourceRequests`) — by default value-type inference: integer-valued `resource.X=N`→`#SBATCH --gres=X:N` (so `--gres=gpu:2` round-trips), empty-valued (feature flags, from `-C`)→`#SBATCH --constraint`, non-integer labels (`cluster`/`host`)→routing-only (emitted as nothing, since they select *which runner* claims, not a node). The `[slurm_runner]` lists `gres_resources`/`gres_exclude` (allow/deny which kinds become `--gres`), `constraint_resources` (kinds whose value emits as a `--constraint` feature), and `nodelist_resources` (kinds emitting as `--nodelist`, e.g. a specific host) override the inference. Multiple entries of one kind combine into a single directive line. The claim response distinguishes `MoreEligible` (a job fit the runner's limits but it lost the claim race — retry) from `Blocked` (queued jobs don't fit the runner's limits/resources); a non-`--forever` runner that is idle (no running jobs, so its offered limits equal full capacity) stops instead of polling forever for `Blocked` jobs it can never satisfy.
- Generic scheduler passthrough: `--custom <key=value>` (repeatable), mirrored by `#BATCHQ -custom key=value`. Stored as `job_details` keys under the `custom.` prefix (`jobs.CustomPrefix`, parsed by `jobs.ParseCustomEntry`) — opaque values batchq's core does not interpret. `procs`/`mem`/`walltime` are reserved names (rejected — use `-p/-m/-t`). A proxy runner translates the keys it recognizes into scheduler flags: the SLURM runner maps `custom.account`→`#SBATCH -A` and `custom.partition`→`#SBATCH -p` in `runner/slurm.go:slurmResourceDirectives`, preferring the job's value and falling back to the runner's `--slurm-account`/`--slurm-partition` (`[slurm_runner] account`/`partition`) default. Under `--slurm`, `#SBATCH -A/--account` and `-p/--partition` are parsed onto `custom.account`/`custom.partition`. Other `custom.*` keys are stored but only emitted by a runner that maps them.
...
- SBATCH header parsing under `--slurm`: `-c/--cpus-per-task`, `--mem`, `-t/--time`, `-J`, `-D`, `-o/-e` with `%j`→`%JOBID`, `--export=ALL`, `-d afterok:…`/`aftercorr:…`, `--array=<spec>`, `-A/--account` → `custom.account`, `-p/--partition` → `custom.partition`, `--gres=name[:type][:count]` → `resource.name[:type]=count`, `-C/--constraint=feat&feat` → bare `resource.feat` flags (simple AND forms only).
...
- `[web]` — `socket`, `listen`, `api` (mount the REST API under `/api/`; `--api` flag also enables it), `username` (HTTP Basic user for the UI, default `admin`), `password` (UI HTTP Basic password; prefer `BATCHQ_PASSWORD` env). `socket` and `listen` are mutually exclusive; a configured `listen` (or the `--listen` flag) now takes precedence over the socket *default* (`web/server.go:chooseTransport`), and the socket default is only applied when no `listen` is set (`support/config.go:ApplyDefaults`).
...
- `[slurm_runner]` — `user`, `account`, `partition` (both are fallback defaults; a job's `custom.account`/`custom.partition` overrides them), `max_jobs` (per-invocation submit cap; `--max-jobs` flag), `max_slurm_jobs` (cap on this user's live SLURM-queue jobs; `--slurm-max-jobs` flag), `min_array` (minimum array batch size — defer batches smaller than this unless it's the array's final remainder; `--slurm-min-array` flag; clamped to the effective job cap), `full_array` (all-or-nothing — only submit an array that fits a single pass entirely; `--slurm-full-array` flag; overrides `min_array`), `gres_resources`/`gres_exclude`/`constraint_resources`/`nodelist_resources` (control how a claimed job's `resource.*` requirements translate into `--gres`/`--constraint`/`--nodelist` sbatch directives — see the resources contract above), plus `[slurm_runner.resources]` (typically cluster/feature labels so resource-tagged jobs route to the right cluster; procs/mem/walltime stay SLURM-enforced).
...
- **Web gateway auth** (`batchq web --api`): the combined gateway adds a single shared *admin account* on top of the token model — HTTP Basic (`admin` + `[web] password`/`BATCHQ_PASSWORD`) gates the browser UI at `/`; the bearer token gates the reverse-proxied `/api/`. Same "shared-secret floor, not per-user identity" philosophy; the reason the UI and API were historically split (avoiding HTTP auth accounts) is now met with one admin password. Middlewares reuse the constant-time sha256 pattern (`web/auth.go`). Still plain HTTP on the wire — TLS remains the reverse proxy's job.
  - **Secure-by-default credential generation.** On a TCP listener (`--listen`/`[web] listen`), any missing credential is *generated* rather than left open: a random `sk-…` API token (when `--api` and no token) and/or a random UI password (when none set) are created with `crypto/rand` (`support.RandomString`/`RandomToken`), printed to the server's stderr, and **persisted to the config file** (`support.SaveConfig`, atomic + `0600`) so restarts reuse the same secret silently. `SaveConfig` writes the raw (pre-defaults) config with `omitempty,omitzero` tags so only set values land in the file; `Duration.MarshalText` keeps durations as `"1m"` strings. Wired in `cmd/web.go`.
